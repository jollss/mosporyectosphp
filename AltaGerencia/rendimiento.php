...
elseif($division==1){
  ...
  if($division!='' & $director==0){

    require ('../conexion.php');

  	$query = "SELECT id_zona,nombre_zona FROM zona where id_zona='$division'";
  	$resultado=$mysqli->query($query);

    while($row = $resultado->fetch_assoc()) {
        $nombrezona=$row['nombre_zona'];
    }
    $result = mysqli_query($mysqli,"SELECT COUNT(*) AS total FROM venta WHERE etapa='ABIERTA'");
    $row = mysqli_fetch_assoc($result);
    $abiertas = $row['total'];

    $resultp = mysqli_query($mysqli,"SELECT COUNT(*) AS posteados FROM venta WHERE etapa ='P'");
    $rowe = mysqli_fetch_assoc($resultp);
    $posteado = $rowe['posteados'];


    $resultc = mysqli_query($mysqli,"SELECT COUNT(*) AS CANCELADA FROM venta WHERE etapa='CANCELADA' OR etapa='CANCELADO'");
    $rowc = mysqli_fetch_assoc($resultc);
    $cancelada = $rowc['CANCELADA'];

    $resultN = mysqli_query($mysqli,"SELECT COUNT(*) AS nada FROM venta WHERE etapa=''");
    $rowN = mysqli_fetch_assoc($resultN);
    $nada = $rowN['nada'];


    $resultduplicada = mysqli_query($mysqli,"SELECT COUNT(*) AS duplicada FROM venta WHERE etapa ='SOLICITUD DUPLICADA'");
    $rowresultduplicada = mysqli_fetch_assoc($resultduplicada);
    $duplicada = $rowresultduplicada['duplicada'];


    $resultcomercial = mysqli_query($mysqli,"SELECT COUNT(*) AS comercial FROM venta WHERE etapa ='comercial'");
    $comercial = mysqli_fetch_assoc($resultcomercial);
    $comerciala = $comercial['comercial'];


    $readeudo = mysqli_query($mysqli,"SELECT COUNT(*) AS adeudo FROM venta WHERE etapa ='ADEUDO'");
    $adeudo = mysqli_fetch_assoc($readeudo);
    $adeudoa = $adeudo['adeudo'];

    $demanda = mysqli_query($mysqli,"SELECT COUNT(*) AS demanda FROM venta WHERE etapa ='DEMANDA/INFRAESTRUCTURA'");
    $demandar = mysqli_fetch_assoc($demanda);
    $demandas = $demandar['demanda'];

    $ninguno = mysqli_query($mysqli,"SELECT COUNT(*) AS ninguno FROM venta WHERE etapa ='0'");
    $ningunor = mysqli_fetch_assoc($ninguno);
    $ningunoa = $ningunor['ninguno'];

$noencontrado = mysqli_query($mysqli,"SELECT COUNT(*) AS noencontrado FROM venta WHERE etapa ='3'");
$noencontradr = mysqli_fetch_assoc($noencontrado);
$noencontradoa = $noencontradr['noencontrado'];


$sinestrategia = mysqli_query($mysqli,"SELECT COUNT(*) AS sinestrategia FROM venta WHERE etapa ='4'");
$sinestrategiar = mysqli_fetch_assoc($sinestrategia);
$sinestrategiaa = $sinestrategiar['sinestrategia'];


$venta = mysqli_query($mysqli,"SELECT COUNT(*) AS venta FROM venta ");
$ventar = mysqli_fetch_assoc($venta);
$ventaa = $ventar['venta'];
/*

---------------------------------------------------empieza count de los usuarios existentes--------------------------------------------------------------------------

*/
$director = mysqli_query($mysqli,"SELECT COUNT(*) AS director FROM usuario AS u INNER JOIN tipo AS t ON t.idtipo=u.tipo_idtipo WHERE tipo='F. DIR. DE AREA' ");
$directorr = mysqli_fetch_assoc($director);
$directora = $directorr['director'];

$coordinador = mysqli_query($mysqli,"SELECT COUNT(*) AS coordinador  FROM usuario AS u
INNER JOIN tipo AS t
ON t.idtipo=u.tipo_idtipo
WHERE tipo='DIR. DE OP. FIELDERS' AND asignado!=0 ");
$coordinadorr = mysqli_fetch_assoc($coordinador);
$coordinadora = $coordinadorr['coordinador'];


$lider = mysqli_query($mysqli,"SELECT COUNT(*) AS lider FROM usuario AS u
INNER JOIN tipo AS t
ON t.idtipo=u.tipo_idtipo
WHERE t.tipo='F. LIDER' AND u.activo='1' ");
$liderr = mysqli_fetch_assoc($lider);
$lidera = $liderr['lider'];

$supervisor = mysqli_query($mysqli,"SELECT COUNT(*) AS supervisor FROM usuario AS u
INNER JOIN tipo AS t
ON t.idtipo=u.tipo_idtipo
WHERE t.tipo='SUP. VENTAS' AND u.activo='1' ");
$supervisorr = mysqli_fetch_assoc($supervisor);
$supervisora = $supervisorr['supervisor'];


$promotor = mysqli_query($mysqli,"SELECT COUNT(*) AS promotor FROM usuario AS u
INNER JOIN tipo AS t
ON t.idtipo=u.tipo_idtipo
WHERE t.tipo='PROMOTOR' ");
$promotorr = mysqli_fetch_assoc($promotor);
$promotora = $promotorr['promotor'];
      ...
      <div class='header'>
          ...
          <div class='menu'>
            ...
            <ul class='nav'>
              ...
              <li class='active'><a href='#'><i class='fas fa-globe-americas fa-2x'></i><i>    $nombrezona</a></li>
          ...
          <div class='profile_details'>
                 <div id='loginContainer'>
                        <a id='loginButton' class=''><span>$nombrezona</span></a>
                          <div id='loginBox'>
                            <form id='loginForm'>
                              <fieldset id='body'>
                                  <div class='user-info'>
                          <h4>Hola,<a href='#'> $nombrezona</a></h4>
                          <ul>

                            <li class='logout'><a href='departamento.php'> Regresar</a></li>
                            <div class='clear'></div>
                          </ul>
                        </div>
                              </fieldset>
   ...
   <div class='main'>
     <div class='wrap'>
        <div class='column_left'>
          <div class='menu_box'>
          <h3>$nombrezona<br>Te Presentamos los 10 estatus </h3>
            <div class='menu_box_list'>
               <ul>

<li ><a href='#'><i class='fas fa-envelope-open fa-2x'></i><span>Abierta</span><label class='digits active'>$abiertas</label><div class='clear'></div></a></li>
<li><a href='#' ><i class='fas fa-times-circle fa-2x'></i><span>DEMANDA/INFRA </span><label class=''>$demandas</label><div class='clear'></div></a></li>
<li><a href='#'><i class='fas fa-copy fa-2x'></i><span>Duplicada</span><label class='digits'>$duplicada</label><div class='clear'></div></a></li>
<li><a href='#'><i class='fas fa-ban fa-2x'></i><span>Cancelada</span><label class='digits'>$cancelada</label><div class='clear'></div></a></li>
<li><a href='#'><i class='fas fa-hands-helping fa-2x'></i><span>Comercial</span><label class='digits'>$comerciala</label><div class='clear'></div></a></li>
<li><a href='#'><i class='fas fa-thumbs-up fa-2x'></i><span>Posteada</span><label class='digits active'>$posteado</label><div class='clear'></div></a></li>
<li><a href='#'><i class='far fa-credit-card fa-2x'></i><span>Adeudo</span><label class='digits'>$adeudoa</label><div class='clear'></div></a></li>
<li><a href='#' ><i class='fas fa-exclamation fa-2x'></i><span>Ninguno</span><label class='digits'>$ningunoa</label><div class='clear'></div></a></li>
<li><a href='#'><i class='fas fa-times fa-2x'></i><span>NO ENCONTRADO</span><label class='digits'>$noencontradoa</label><div class='clear'></div></a></li>
<li><a href='#' ><i class='fas fa-chess-queen fa-2x'></i><span>SIN ESTRATEGIA</span><label class='digits'>$sinestrategiaa</label><div class='clear'></div></a></li>
             </ul>
           </div>
 ...
 <div class='profile_picture'>
  <a href=''><img src='images/profile_img150x150.jpg' alt='' />	</a>
    <div class='profile_picture_name'>
      <h2>$nombrezona</h2>
      <p>Bienvenido, El TOTAL DE TUS VENTAS ES DE $ventaa ventas </p>
    </div>
     <span><a href='#'> <img src='images/follow_user.png' alt='' /> </a></span>
  </div>
 <div class='articles_list'>
      <ul>
        <li><a href='#' class='red'>Reporte de Todas las Ventas de $nombrezona</a></li>
        <li><a href='#' class='purple'>Ventas Sin Validar Aun <br>$nada<br>Consultalas aqui</a></li>
        <li><a href='#' class='yellow'>Realizar Consulta de $nombrezona</a></li>
        <div class='clear'></div>
      </ul>
   </div>
   </div>
...
<div class='column_right'>
     ...
     <div class='newsletter'>
       <div class='menu_box'>
         <h3>$nombrezona total de Personal Activo</h3>
           <div class='menu_box_list'>
              <ul>
              <li><a href='#'><i class='fas fa-users fa-2x'></i><span>Director</span><label class='digits'>$directora</label><div class='clear'></div></a></li>
              <li><a href='#'><i class='fas fa-users fa-2x'></i><span>Coordinador</span><label class='digits active'>$coordinadora</label><div class='clear'></div></a></li>
              <li><a href='#'><i class='fas fa-users fa-2x'></i><span>Lider</span><label class='digits'>$lidera</label><div class='clear'></div></a></li>
              <li><a href='#'><i class='fas fa-users fa-2x'></i><span>Supervisor de Ventas</span><label class='digits'>$supervisora</label><div class='clear'></div></a></li>
  <li><a href='#'><i class='fas fa-users fa-2x'></i><span>Promotores</span><label class='digits'>$promotora</label><div class='clear'></div></a></li>
...
";
/*  $estatus = array("P ","ABIERTA");
for ($i = 0; $i < 2; $i++) {

for ($j = 0; $j < sizeof($estatus); $j++) {
echo ". $array[$j] .<br>";
}
//echo "$i<br>";
}
...
Highcharts.chart('container', {
    ...
    for($i=0; $i<$longitud; $i++)
          ...
          $estatusmeses = mysqli_query($mysqli,"SELECT COUNT(*) AS ptotal FROM venta WHERE etapa='$sestatus' AND mes='$j'");
...
<?php
    ...
    elseif($division!='' & $director!='' & $coordinador==0){
      //////////////////////////////////////////////////////////////////////////////////////empezo director////////////////////////////////////////////////////////////////////////////////////////////////777
require ('../conexion.php');
$result = mysqli_query($mysqli,"SELECT COUNT(*) AS total FROM venta WHERE etapa='ABIERTA'");
$row = mysqli_fetch_assoc($result);
...
$ventaa = $ventar['venta'];
////////////////////////////////////////////////////////////////////
  $nombrecoordinador = "SELECT u.idu,u.nombre,t.tipo,u.id_zona FROM usuario AS u
INNER JOIN tipo AS t
ON t.idtipo=u.tipo_idtipo


WHERE t.tipo='F. DIR. DE AREA' AND u.idu='$director'";
  $resultado=$mysqli->query($nombrecoordinador);
  ...
  <div class='header'>
      ...
      <div class='menu'>
        ...
        <ul class='nav'>
          ...
          <li class='active'><a href='#'><i class='fas fa-globe-americas fa-2x'></i><i>    $nombrec</a></li>
      ...
      <div class='profile_details'>
             <div id='loginContainer'>
                    <a id='loginButton' class=''><span>$nombrec</span></a>
                      <div id='loginBox'>
                        <form id='loginForm'>
                          <fieldset id='body'>
                              <div class='user-info'>
                      <h4>Hola,<a href='#'> $nombrec</a></h4>
                      <ul>
...
<div class='main'>
 <div class='wrap'>
    <div class='column_left'>
      <div class='menu_box'>
      <h3>$nombrec<br>Te Presentamos los 10 estatus </h3>
        <div class='menu_box_list'>
           <ul>
...
<div class='articles_list'>
  <ul>
    <li><a href='#' class='red'>Reporte de Todas las Ventas de $nombrec</a></li>
    <li><a href='#' class='purple'>Ventas Sin Validar Aun <br>$nada<br>Consultalas aqui</a></li>
    <li><a href='#' class='yellow'>Realizar Consulta de $nombrec</a></li>
    <div class='clear'></div>
  </ul>
</div>
</div>
...
<div class='column_right'>
 ...
 <div class='newsletter'>
   <div class='menu_box'>
     <h3>$nombrec total de Personal Activo</h3>
       <div class='menu_box_list'>
          <ul>
            <li><a href='#'><i class='fas fa-users fa-2x'></i><span>Coordinador</span><label class='digits active'>$coordinadora</label><div class='clear'></div></a></li>
          <li><a href='#'><i class='fas fa-users fa-2x'></i><span>Lider</span><label class='digits'>$lidera</label><div class='clear'></div></a></li>
...
/*------------------------------------------------------------------ Termina Director------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    ...
    elseif($division!='' & $director!='' & $coordinador!='' & $lider==0){
...
ON u.idu=v.idvendedor where etapa='$sestatus' AND mes='$j' and u.coordinador='$coordinador'");
  ...
  ";
    ...
    elseif($division!='' & $director!='' & $coordinador!='' & $lider!='' & $supervisor==0){
      require ('../conexion.php');
...
ON u.idu=v.idvendedor
 ...
 WHERE  u.coordinador='$coordinador' AND u.asignado='$lider'");
    ...
    elseif($division!='' & $director!='' & $coordinador!='' & $lider!='' & $supervisor!=0 & $promotor==0){
      require ('../conexion.php');
      /*------------------------------------------------------------------ Consulta Nombre supervisor------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    ...
    WHERE idu='$promotor'";
      ...
      INNER JOIN venta AS v
      ON v.idvendedor=u.idu
      WHERE v.etapa='P'  AND u.coordinador='$coordinador' AND u.pertenece_a='$supervisor' and u.asignado='$supervisor' and idu='$promotor'");
      $rowconCooP = mysqli_fetch_assoc($consultadeventacoordinadorP);
      $totalventacoordinadorP = $rowconCooP['totalventacoordinadorP'];
      /////////////////////////////////
      $consultadeventacoordinadorADEUDO= mysqli_query($mysqli,"SELECT COUNT(*) AS totalventacoordinadorADEUDO FROM usuario AS u
      ...
      <div class='header'>
          ...
          <div class='menu'>
            ...
            <button type='button' class='btn btn-primary' id='btn-cancelar'>
             <li>
             <a href='#'><i><img src='images/settings.png' alt='' /></i>
             <p style='color:Black'>Cambiaste La seleccion de un option value se registro tu ip</p>
          </button></a></li>
              <script type='text/javascript'>
                 document.getElementById('btn-cancelar').onclick = function () {
                 location.href = 'departamento.php';
