  ...
  && isset($tel2)&& isset($folio)&& isset($tel3))
{
  ...
  $sql="UPDATE ventas SET 
            vendedor='".$vende."'
          WHERE idventa='".$ultimo."'";
if ($con->query($sql) === TRUE) { echo "update<br>"; } else { if (!mysqli_query($con, $sql)) { printf("Errormessage: %s\n", mysqli_error($con)); echo "<br>";} }
  ...
  $sql="INSERT INTO ubicacion_venta 
             (idubicacion,longitud,latitud,idventa)
              VALUES
  ('".$ultimo."','".$longitud."','".$latitud."','".$ultimo."')"; 
  if ($con->query($sql) === TRUE) { echo "insert en ubicacion_venta<br>"; } else { if (!mysqli_query($con, $sql)) { printf("Errormessage: %s\n", mysqli_error($con)); echo "<br>";} }
