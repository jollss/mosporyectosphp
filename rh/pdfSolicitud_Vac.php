...
foreach ($_POST['array_selecion'] as $valor) {  
    ...
    while($row = $resultado->fetch_assoc())
    {
		...
		/*===========================*/
			...
			$aaaa=date('Y');
			$vigencia=$emision;
			//$vigencia=$dia."-".$mes."-".$aaaa;
			$area=strtoupper($_POST['area']);
			$del=date("Y", strtotime($del))."-".date("m", strtotime($del))."-".date("d", strtotime($del));
			$final=date("Y", strtotime($final))."-".date("m", strtotime($final))."-".date("d", strtotime($final));
			$periodo=$del." al ".$final;
			$dias=dateDiff($del, $final);
			//$dias=0;//$_POST['dias_t'];
		...
		/**************/
			...
			$pdf->SetFont('Arial','',9);
			$pdf->Cell(63.3,9,utf8_decode($visto),0,0,'C');
			$pdf->Cell(63.3,9,utf8_decode(""),0,0,'L');
			$pdf->Cell(63.3,9,utf8_decode("FIRMA DEL SOLICITANTE"),0,1,'C');
			$pdf->Cell(190,10,"",0,1,'L');
