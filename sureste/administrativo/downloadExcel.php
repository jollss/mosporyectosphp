...
if($tipo==3){
    ...
    /*$sql="SELECT * FROM os 
        ...
        WHERE estatus=1 and idmos=id_orden and idos=idmos AND cope='$place'";
    }if($tipo_user==39){
        //VILLA HERMOSA
        $place="CT VLL";
        /*$sql="SELECT * FROM os 
        ...
        $sql="SELECT * FROM os inner join dataos inner join material
        WHERE estatus=1 and idmos=id_orden and idos=idmos AND cope='$place' OR cope='$place2'";
    }
    ...
    while($row = $resultado->fetch_assoc())
    {
        ...
        if (check_in_range($start_date, $end_date, $fecha_a_evaluar)) {
            ...
            if($asignado<>0 and $idmos<>0){
                ...
                $usuarioidu2=$row['supervisor_idu'];
                $con3 = Conectarse();
                $sql3="SELECT * FROM usuario WHERE idu='$usuarioidu2' and tipo_idtipo=3";
                $resultado3=$con3->query($sql3);
                while($row3 = $resultado3->fetch_assoc())
                {
                    $nms=$row3['nombre'];
                ...
                if(!isset($nms) or $nms==''){
                    $con34 = Conectarse();
                    $sql34="SELECT * FROM usuario WHERE idu='$usuarioidu2' and tipo_idtipo=3";
                    $resultado34=$con34->query($sql34);
                    while($row4 = $resultado34->fetch_assoc())
                    {
                        $nms=$row4['nombre'];
                        $apms=$row4['apaterno'];
                        $ams=$row4['amaterno']; 
