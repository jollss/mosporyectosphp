...
$tipo_os_anterior=strtoupper($_POST['tipo_os_anterior']);
$idc=0;
      $id=0;
      echo $os."-".$cope."-".$expediente."-".$folio_pisa."-".$folio_pisaplex."-".$telefono."-".$cliente."-".$tipo_tarea."-".$distrito."-".$zona."-".
      $dilacion_etapa."-".$dilacion."-".$tipo_os."-".$tipo_os_anterior."-";
...
if(isset($iduser) && isset($os))
{
  ...
  //$orden->registrarOsBD($con);
    $es=0;//$orden->existe($folio_pisa,$con);
    //echo "ID:".$id;
    if($es==1){
        echo "
        <script>
      ...
      if ($_SERVER['REQUEST_METHOD'] == 'POST')
      {
        ...
        //$cantidad->actualizaCantidadesMenosBD($tipo_os_anterior,$iduser,$con);
